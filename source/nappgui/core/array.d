...
Array *array_create(uint16_t esize, const char_t *type);
...
Array *array_read(Stream *stream, uint16_t esize, FPtr_read_init func_read_init, const char_t *type);
...
byte_t *array_get(const Array *array, uint32_t pos);

byte_t *array_get_last(const Array *array);

byte_t *array_all(const Array *array);

byte_t *array_insert(Array *array, uint32_t pos, uint32_t n);

byte_t *array_insert0(Array *array, uint32_t pos, uint32_t n);
...
void array_delete(Array *array, uint32_t pos, uint32_t n, FPtr_remove func_remove);

void array_delete_ptr(Array *array, uint32_t pos, uint32_t n, FPtr_destroy func_destroy);
