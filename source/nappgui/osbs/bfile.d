 ...
 * 
 * See_Also: https://nappgui.com/en/osbs/bfile.html
 */
module nappgui.osbs.bfile;
...
uint32_t bfile_dir_work(char_t *pathname, uint32_t size);

bool_t bfile_dir_set_work(const char_t *pathname, ferror_t *error);

uint32_t bfile_dir_home(char_t *pathname, uint32_t size);

uint32_t bfile_dir_data(char_t *pathname, uint32_t size);

uint32_t bfile_dir_exec(char_t *pathname, uint32_t size);
...
bool_t bfile_dir_get(Dir *dir, char_t *name, uint32_t size, file_type_t *type, uint64_t *fsize, Date *updated, ferror_t *error);
...
bool_t bfile_read(File *file, byte_t *data, uint32_t size, uint32_t *rsize, ferror_t *error);

bool_t bfile_write(File *file, const byte_t *data, uint32_t size, uint32_t *wsize, ferror_t *error);

bool_t bfile_seek(File *file, int64_t offset, const file_seek_t whence, ferror_t *error);
